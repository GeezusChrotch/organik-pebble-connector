...
Configure Xcode Cloud with stable **macOS 26** and **Xcode 26.6 (17F113)**, a clean environment, restricted editing, and an **Archive — TestFlight and App Store** action. Set the next build number to **40 or higher**. Do not select beta macOS or beta Xcode: Apple rejected those build environments. No build-provenance fields are overridden. Set no signing-certificate or private-key secrets; use Apple-managed signing.
...
The next release pins the accepted local39 source in `ACCEPTED-SOURCE.json`. Pome uses direct Apple Home for controls and cameras; no Itsyhome dependency. See `NETWORK-SERVICES.md` for the incoming-server entitlement explanation and reviewer steps.

...
p=pathlib.Path(sys.argv[1])/'Contents/Info.plist';d=plistlib.loads(p.read_bytes());d['NotesyPackageVersion']='1.4.8';d.update(CFBundleShortVersionString='1.0.0',CFBundleVersion='106',OrganikUpgradeModel='owned-v1',SUEnableInstallerLauncherService=True,OrganikDistribution='direct-download',OrganikPomeAvailable=False)

...
@MainActor final class ConnectorModel: ObservableObject {
    ...
    func start() {
        for service in [stone.objectWillChange, pome.objectWillChange, tesla.objectWillChange] {
    ...
    func refresh() async {
        ...
        if visiblePages.contains(.pome) { await pome.check() }
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let checks = self.visiblePages.map { page in
                ["app": page.rawValue, "requirements": self.requirements(page).map {
                    ["id": $0.id, "title": $0.title, "ready": $0.ready, "detail": $0.id == "route" ? $0.detail : ""] as [String: Any]
                }] as [String: Any]
            }
    ...
    func shutdown() { timer?.invalidate(); stone.shutdown(); reminderz?.stopModule() }
}
...
struct RequirementLight: View {
    ...
    var body: some View {
        HStack(spacing: 7) {
            Circle().fill(requirement.ready ? Color.green : Color.red).frame(width: 9, height: 9)
            Text(requirement.title).font(.callout)
        }
        .help((requirement.ready ? "Ready: " : "Needs attention: ") + requirement.detail)
        .accessibilityElement(children: .ignore)
        .accessibilityLabel(requirement.title + (requirement.ready ? ": ready" : ": needs attention"))
    }
}
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                Text(requirements.allSatisfy(\.ready) ? "Your Mac is ready. Connect your phone below, or review your setup." : "Set up this app by following the steps below. Existing settings and pairing are reused.")
...
struct BeepsterView: View {
    ...
    var body: some View {
        ConnectorDetail(page: .beepster, requirements: module.requirements, busy: module.busy, message: module.message) {
            ...
            SetupStep(number: 2, title: "Allow contact names and set up the service", detail: "Allow Contacts when macOS asks so Beepster can show names. Set up service installs the bundled service and checks your Beeper connection.") {
                HStack {
...
struct ConnectorWindow: View {
    ...
    var body: some View {
        NavigationSplitView {
                        ...
                        ForEach(model.visiblePages) { page in
                            GroupBox {
                                HStack(alignment: .top, spacing: 18) {
                                    ...
                                    if model.requirements(page).contains(where: { !$0.ready }) { Button("Fix") { model.selection = page } }
                                }.padding(12)
                            }
                        }
                    }.padding(28).frame(maxWidth: 850).frame(maxWidth: .infinity, alignment: .leading)
            ...
            case .pome: ExternalServiceView(page: .pome, service: model.pome)

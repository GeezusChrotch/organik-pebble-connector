...
final class ReminderzModule: NSObject, NSWindowDelegate, ObservableObject {
    @Published var requirements: [ConnectorRequirement] = [
        ConnectorRequirement("permission", "Reminders access", false, "Not checked"),
    ...
    func setUpSync() {
        if EKEventStore.authorizationStatus(for: .reminder) != .fullAccess { requestReminders(); return }
    ...
    private func startService() {
        ...
        server.start { [weak self] result in
            ...
            case .failure:
                ...
                self.refresh()
                self.setStatus(self.connectorStatus, ok: false, text: "Mac service unavailable. If the standalone Reminderz Connector is running, stop its service before starting this one. Existing pairing is preserved.")
            }
        }
    }
    ...
    private func buildContent() -> NSView {
        ...
        let title = NSTextField(labelWithString: "Reminderz")
            ...
            DispatchQueue.main.async {
                ...
                if !granted {
                    let detail = error?.localizedDescription ??
                        "Open System Settings → Privacy & Security → Reminders and allow Organik Apps Pebble Connector."
    ...
    @objc private func stopPrivateSync() {
        ...
        alert.informativeText = "This removes only the Reminderz HTTPS route on port \(activePort). Other Tailscale Serve routes stay unchanged."
    ...
    private func refresh() {
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            let privateResult = origin.map { Self.healthResult(token: token, origin: $0) } ?? (ready: false, detail: "No private route is configured for Reminderz.")
    ...
    private static func localHealth(token: String) -> Bool {
        healthResult(token: token, origin: "http://127.0.0.1:7843").ready
    }
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let error = error as NSError? {
                ...
                result = (false, "The service returned HTTP \(http.statusCode).")
                      ...
                      health["ok"] as? Bool == true && health["apiVersion"] as? Int == 1 {
                result = (true, "Private route reached Reminderz from this Mac.")
            } else { result = (false, "The route did not return Reminderz health data.") }
        }
        task.resume()

...
export function createBridge(config, { upstream = 'http://127.0.0.1:7855', fetcher = fetch, beepsterUpstream = 'http://127.0.0.1:8794', eventzUpstream = 'http://127.0.0.1:7848', assets = path.join(path.dirname(fileURLToPath(import.meta.url)), 'pome/dist') } = {}) {
  config = {...config, homeToken: ''}; // Direct distribution never exposes HomeKit, even with saved credentials.
  ...
  return http.createServer({maxHeaderSize:8192, requestTimeout:70000, headersTimeout:10000}, async (req, res) => {
    ...
    try {
      ...
      if (!config.homeToken) return json(503,{error:'Pome: Coming to the Apple App Store soon'});

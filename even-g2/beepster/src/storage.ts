...
export const settingsKey='org.organikapps.beepster.settings.v1';
...
export class SettingsStore {
 ...
 async load():Promise<Settings>{
  ...
  let legacy:string|null=null;try{legacy=this.browser.getItem('beepster.settings');}catch{}
  const settings=legacy?this.decode(legacy):loadSettings('{}');
  if(legacy){if(!await this.native.setLocalStorage(settingsKey,JSON.stringify(settings)))throw new Error('Even could not preserve the existing settings. Reopen Beepster and retry.');}
 ...
 async save(settings:Settings):Promise<void>{
  ...
  if(!await this.native.setLocalStorage(settingsKey,raw))throw new Error('Even could not save your settings. Please retry before closing Beepster.');
  try{this.browser.setItem('beepster.settings',raw);}catch{} // Optional cache; native storage is authoritative.
 }
}
